...
Os dados estão organizados em uma tabela definida como `df_master`, que contém uma série de informações sobre espécies nativas de plantas brasileiras:
...
Das 12 colunas dessa tabela, vou usar as informações contidas em cinco:
...
Vou fazer minhas análises e construir as visualizações com base nessas informações e derivações das mesmas.

# Descrições ao longo do tempo
***

Como botânico, sei que o número de descrições de plantas brasileiras vem aumentando ao longo dos anos. Mas quero entender como que esses números se comportam ao longo do tempo. Para isso agrupei o número de espécies descritas por ano e plotei um gráfico de linhas. Já aproveitei o trabalho e dividi as informações por século, colocando diferentes cores para cada:
...
Existe outra característica interessante nesse plot. Os estados foram ordenados, propositalmente, de acordo com suas regiões. Os estados localizados na parte inferior do gráfico são todos do sudeste do Brasil e pode-se ver uma clara concentração de valores altos de densidade de descrições, entre eles. À parte destes, apenas os estados da Bahia e do Amazonas parecem ter uma concentração significante o suficiente para ser vista, no gráfico pelo menos, o que indica um desbalanço no número de coletas entre os estados brasileiros.
