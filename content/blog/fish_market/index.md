...
Vou definir como minha variável de resposta o peso do peixe, esse é o valor que quero prever em função das demais variáveis explicativas. Para dar início à análise, então, vou plotar a correlação de minha variável de resposta com todas as demais variáveis explicativas e calcular o `\(R^2\)` de cada caso.
...
Essa tabela me dá os valores de `\(R^2\)` para cada modelo de peso em função a cada uma das demais variáveis, de maneira independente.
...
Vou desenvolver isso um pouco mais a frente, mas por hora, como vou fazer uma regressão simples, vou usar a variável `Length3` como explicativa, dado que ela resultou no maior valor de `\(R^2\)`.
